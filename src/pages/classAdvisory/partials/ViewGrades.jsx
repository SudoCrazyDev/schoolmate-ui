...
export default function ViewGrades({student, subjects, advisory}){
    ...
    const [selectedSchoolDay, setSelectedSchoolDays] = useState(null);
    ...
    const handleFetchSchoolDays = async () => {
        setFetching(true);
    ...
    const handleGetStudentAttendance = (month) => {
        ...
        let default_school_days = selectedSchoolDay ? JSON.parse(selectedSchoolDay) : [];
        ...
        return {
            ...
            absent:  isNaN(parseInt(default_school_days[month]) - parseInt(attendance_records[month])) ? "" : IsNegative(parseInt(default_school_days[month]) - parseInt(attendance_records[month])) ? "-" : parseInt(default_school_days[month]) - parseInt(attendance_records[month]),
            total_school_days: isNaN(total_school_days) ? "" : total_school_days,
            total_present: isNaN(total_present) ? "" : total_present,
            total_absent: isNaN(total_school_days - total_present) ? "" : IsNegative(Number(total_school_days - total_present)) ? 0 : total_school_days - total_present
        };
    ...
    useEffect(() => {
        if(open){
            ...
            handleFetchSchoolDays();
        }
    }, [open]);
    
    useEffect(() => {
        if(schoolDays){
            setSelectedSchoolDays(JSON.stringify(schoolDays[0]));
        }
    },[schoolDays]);
    
    useEffect(() => {
        if(cardTemplates){
            setSelectedTemplate(cardTemplates?.[0]?.subjects);
        }
    },[cardTemplates]);
                ...
                <DialogContent dividers>
                    <div className="d-flex flex-row gap-2 h-100">
                        <div className="col-6 d-flex flex-column p-5">
                            ...
                            <p className="text-black m-0 mt-3">School Days Template</p>
                            <select className="form-select" onChange={(e) => setSelectedSchoolDays(e.target.value)}>
                                {schoolDays.map(schoolDay => {
                                    return <option key={schoolDay.id} value={JSON.stringify(schoolDay)}>
                                        {schoolDay.academic_year}
                                    </option>
                                })}
                            </select>
                            <p className="text-black m-0 mt-3">Custom Data</p>
                            <span className="fst-italic text-muted">Note: Overriding Current Data, Data will not be saved.</span>
                            <div className="d-flex flex-row mt-3">
                                <div className="d-flex flex-column">
                                    <p className='m-0'>Age:</p>
                                    <input type="number" className='form-control' value={ageOverride} onChange={(e) => setAgeOverride(e.target.value)}/>
                                </div>
                            </div>
                        </div>
                        <div className="col-6">
                            {fetching && (
                                <div className='d-flex flex-column h-100 align-items-center justify-content-center gap-2'>
                                    ...
                                    <h5 className='fst-italic text-muted'>Generating Card...</h5>
                                </div>
                            )}
                            {!fetching && selectedTemplate && (
                                            ...
                                            <View style={{width: '50%', padding: '20px', paddingTop: '8px'}}>
                                                ...
                                                <View style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                                                    ...
                                                    <View style={{marginTop: '3px', display: 'flex', flexDirection: 'row', alignSelf: 'flex-start', gap: '20px'}}>
                                                        <Text style={{fontFamily: 'Helvetica-Bold', fontSize: '8px'}}>Age: <Text style={{textDecoration: 'underline'}}>{ageOverride ? ageOverride : calculateAge(student.birthdate)}</Text></Text>
